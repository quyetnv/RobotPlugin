 ...
 */
package vn.fpt.robot.plugin.templates;

import static com.android.SdkConstants.ATTR_ID;
import static com.android.ide.eclipse.adt.AdtUtils.extractClassName;
import static vn.fpt.robot.plugin.templates.NewTemplatePage.WIZARD_PAGE_WIDTH;
...
import vn.fpt.robot.plugin.newproject.ApplicationInfoPage;
import vn.fpt.robot.plugin.newproject.ProjectNamePage;
import vn.fpt.robot.plugin.utils.Utils;
 ...
 */
public class NewRobotProjectPage extends WizardPage implements ModifyListener,
		SelectionListener, FocusListener {
	private static final int FIELD_WIDTH = 300;
	private static final String SAMPLE_PACKAGE_PREFIX = ""; //$NON-NLS-1$
	/** Suffix added by default to activity names */
	static final String ACTIVITY_NAME_SUFFIX = "RobobtActivity"; //$NON-NLS-1$
	/** Prefix added to default layout names */
	static final String LAYOUT_NAME_PREFIX = "robot_activity_"; //$NON-NLS-1$
	private static final int INITIAL_MIN_SDK = 14;
	...
	NewRobotProjectPage(NewProjectWizardState values) {
		super("newAndroidApp"); //$NON-NLS-1$
		mValues = values;
		setTitle("New Robot Application");
		setDescription("Creates a new Robot Application");
	}
	...
	public void createControl(Composite parent) {
		...
		GridData gdApplicationText = new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 2, 1);
		...
		GridData gdProjectText = new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 2, 1);
		...
		packageLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 2, 1));
		packageLabel.setText("Package Name:");

		mPackageText = new Text(container, SWT.BORDER);
		GridData gdPackageText = new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 2, 1);
		...
		mPackageDec = createFieldDecoration(
				mPackageText,
		...
		minSdkLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 2, 1));
		minSdkLabel.setText("Minimum Required SDK:");

		mMinSdkCombo = new Combo(container, SWT.READ_ONLY);
		GridData gdMinSdkCombo = new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 1, 1);
		...
		for (IAndroidTarget target : targets) {
			...
			if (target.isPlatform()
					&& target.getVersion().getApiLevel() <= AdtUtils
							.getHighestKnownApiLevel()) {
				targetLabel = AdtUtils.getAndroidName(target.getVersion()
						.getApiLevel());
			} else {
		...
		for (int i = 0, n = targets.length; i < n; i++) {
			...
			if (version.isPreview()) {
				...
				mMinNameToApi.put(targetLabel, apiLevel);
					...
					&& (mValues.target == null || apiLevel > mValues.target
		...
		for (String label : AdtUtils.getKnownVersions()) {
			if (Utils.toVersionAPI(label) >= 14) {
				labels.add(label);
			}
		}
		assert labels.size() >= 15; // *Known* versions to ADT, not
		...
		if (mValues.target != null && mValues.target.getVersion().isPreview()) {
			...
			System.out.println("IF:" + mValues.minSdk);
			mMinSdkCombo.setText(mValues.minSdk);
			mValues.iconState.minSdk = mValues.target.getVersion()
					.getApiLevel();
			mValues.minSdkLevel = mValues.iconState.minSdk;
		} else {
			mMinSdkCombo.select(INITIAL_MIN_SDK - 14);
			mValues.minSdk = Integer.toString(INITIAL_MIN_SDK);
			System.out.println("ELSE:" + mValues.minSdk);
		...
		mMinSdkDec = createFieldDecoration(
				mMinSdkCombo,
		...
		buildSdkLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 2, 1));
		buildSdkLabel.setText("Compile With:");

		mBuildSdkCombo = new Combo(container, SWT.READ_ONLY);
		GridData gdBuildSdkCombo = new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 1, 1);
		gdBuildSdkCombo.widthHint = FIELD_WIDTH;
		mBuildSdkCombo.setLayoutData(gdBuildSdkCombo);
		mBuildSdkCombo.setData(targets);
		mBuildSdkCombo.setItems(targetLabels.toArray(new String[targetLabels
				.size()]));
		...
		mBuildTargetDec = createFieldDecoration(
				mBuildSdkCombo,
		...
		if (metadata != null) {
			...
			if (mThemeParameter != null && mThemeParameter.element != null) {
				...
				themeLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER,
						false, false, 2, 1));
				themeLabel.setText("Theme:");

				mThemeCombo = NewTemplatePage.createOptionCombo(
						mThemeParameter, container, mValues.parameters, this,
						this);
				GridData gdThemeCombo = new GridData(SWT.LEFT, SWT.CENTER,
						true, false, 1, 1);
		...
		mTipLabel.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 3,
				1));
	 ...
	 * and min sdk targets. Also runs {@link #validatePage} in case no valid
	 * entry was found. Does nothing if called on a template that does not
	 * supply a theme.
	 */
	void updateTheme() {
		if (mThemeParameter != null) {
			// Pick the highest theme version that works for the current SDK
			// level
			...
			if (parameter != null) {
				...
				for (int index = optionIds.length - 1; index >= 0; index--) {
					IStatus status = NewTemplatePage.validateCombo(null,
							mThemeParameter, index, mValues.minSdkLevel,
							mValues.getBuildApi());
					if (status == null || status.isOK()) {
						String optionId = optionIds[index];
						parameter.value = optionId;
						parameter.edited = optionId != null
								&& !optionId.toString().isEmpty();
	...
	private IAndroidTarget[] getCompilationTargets() {
		...
		for (IAndroidTarget target : targets) {
			if (target.isPlatform() == false
					&& (target.getOptionalLibraries() == null || target
							.getOptionalLibraries().length == 0)) {
				continue;
			}
			list.add(target);
		}
	...
	private void updateActivityNames(String name) {
		try {
			...
			if (!mValues.activityNameModified) {
				mValues.activityName = extractClassName(name)
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (source == mMinSdkCombo) {
			...
			if (mValues.minSdkLevel > mValues.getBuildApi()) {
				...
				IAndroidTarget[] targets = (IAndroidTarget[]) mBuildSdkCombo
						.getData();
	...
	private void setSelectedTargetSdk(int api) {
		mTargetSdkCombo.select(api - 1); // -1: First API level (at index 0) is
											// 1
	}
	...
	private void validatePage() {
		...
		if (status != null && !status.isOK()) {
			...
			if (appStatus != null
					&& (status == null || appStatus.getSeverity() > status
							.getSeverity())) {
				status = appStatus;
			}
			...
			if (projectStatus != null
					&& (status == null || projectStatus.getSeverity() > status
							.getSeverity())) {
			...
			if (packageStatus != null
					&& (status == null || packageStatus.getSeverity() > status
							.getSeverity())) {
				status = packageStatus;
			}

			IStatus locationStatus = ProjectContentsPage
					.validateLocationInWorkspace(mValues);
			if (locationStatus != null
					&& (status == null || locationStatus.getSeverity() > status
							.getSeverity())) {
			...
			if (themeStatus != null
					&& (status == null || themeStatus.getSeverity() > status
							.getSeverity())) {
				status = themeStatus;
			}
		}
	...
	private IStatus validateProjectName() {
		IStatus status = ProjectNamePage
				.validateProjectName(mValues.projectName);
	...
	private IStatus validatePackageName() {
		...
		if (mValues.packageName == null
				|| mValues.packageName.startsWith(SAMPLE_PACKAGE_PREFIX)) {
			if (mValues.packageName != null
					&& !mValues.packageName.equals(SAMPLE_PACKAGE_PREFIX)) {
				status = ApplicationInfoPage
						.validatePackage(mValues.packageName);
				if (status == null || status.isOK()) {
					status = new Status(IStatus.WARNING, AdtPlugin.PLUGIN_ID,
							String.format(
									"The prefix '%1$s' is meant as a placeholder and should "
											+ "not be used",
	...
	private void updateDecorator(ControlDecoration decorator, IStatus status,
			boolean hasInfo) {
		if (hasInfo) {
	...
	private void setDecoratorType(ControlDecoration decorator, int severity) {
		...
		FieldDecoration errorFieldIndicator = FieldDecorationRegistry
				.getDefault().getFieldDecoration(id);
